...
mcp = FastMCP(name="Jira MCP", host="127.0.0.1", port=3333)
...
async def GET_ISSUE(issue_key: str) -> str:
    ...
    # Handle description - can be ADF (dict) or plain text (str)
...
def _extract_text_from_adf(adf_content: Dict) -> str:
    ...
    Args:
        adf_content: ADF formatted content (dict)
...
async def CREATE_ISSUE(
    ...
    Args:
        ...
        priority: Priority level (default: Medium, leave empty to skip)
    """
    ...
    # Only add priority if it's provided and not empty
...
async def UPDATE_ISSUE(
    ...
    data = {"fields": fields}
    result = await make_jira_request("PUT", f"/rest/api/3/issue/{issue_key}", data)
...
async def ADD_COMMENT(issue_key: str, comment: str) -> str:
    ...
    """
    # API v3 uses different comment format
    data = {
        "body": {
            "type": "doc",
            "version": 1,
...
async def TRANSITION_ISSUE(issue_key: str, transition_name: str) -> str:
    ...
    transitions_result = await make_jira_request("GET", f"/rest/api/3/issue/{issue_key}/transitions")
    ...
    result = await make_jira_request("POST", f"/rest/api/3/issue/{issue_key}/transitions", data)
...
async def GET_ISSUE_TYPES() -> str:
    ...
    result = await make_jira_request("GET", "/rest/api/3/issuetype")
...
async def GET_PRIORITIES() -> str:
    ...
    result = await make_jira_request("GET", "/rest/api/3/priority")
